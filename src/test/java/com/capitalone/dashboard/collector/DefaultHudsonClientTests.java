...
public class DefaultHudsonClientTests {
	...
	public void init() {
		...
		hudsonClient = defaultHudsonClient = new DefaultHudsonClient(restOperationsSupplier, settings);
	}
	...
	public void joinURLsTest() throws Exception {
		...
		String u4 = DefaultHudsonClient.joinURL("http://jenkins.com/",
				new String[] { "test", "/api/json?tree=jobs[name,url,builds[number,url]]" });
		assertEquals("http://jenkins.com/test/api/json?tree=jobs[name,url,builds[number,url]]", u4);

		String u2 = DefaultHudsonClient.joinURL("http://jenkins.com/",
				new String[] { "/test/", "/api/json?tree=jobs[name,url,builds[number,url]]" });
		assertEquals("http://jenkins.com/test/api/json?tree=jobs[name,url,builds[number,url]]", u2);

		String u3 = DefaultHudsonClient.joinURL("http://jenkins.com",
				new String[] { "///test", "/api/json?tree=jobs[name,url,builds[number,url]]" });
	...
	public void rebuildURLTest() throws Exception {
		...
		String u1 = DefaultHudsonClient.rebuildJobUrl("http://jenkins.com/job/job1",
				"https://123456:234567@jenkins.com");
		assertEquals("https://123456:234567@jenkins.com/job/job1", u1);

		String u2 = DefaultHudsonClient.rebuildJobUrl("https://jenkins.com/job/job1",
				"https://123456:234567@jenkins.com");
		assertEquals("https://123456:234567@jenkins.com/job/job1", u2);

		String u3 = DefaultHudsonClient.rebuildJobUrl("http://jenkins.com/job/job1",
				"http://123456:234567@jenkins.com");
		assertEquals("http://123456:234567@jenkins.com/job/job1", u3);

		String u4 = DefaultHudsonClient.rebuildJobUrl("http://jenkins.com/job/job1",
				"http://123456:234567@jenkins.com");
	...
	public void verifyAuthCredentials() throws Exception {
		// TODO: This change to clear a JAVA Warning should be correct but test fails,
		// need to investigate
	...
	public void verifyAuthCredentialsBySettings() throws Exception {
		// TODO: This change to clear a JAVA Warnings should be correct but test fails,
		// need to investigate
		// HttpEntity<HttpHeaders> headers = new
	...
	public void instanceJobs_twoJobsTwoBuilds() throws Exception {
		when(rest.exchange(eq(URI.create("http://server/job/job2/2/api/json?tree=jobs")), eq(HttpMethod.GET),
		...
		when(rest.exchange(eq(URI.create(
				"http://server/job/job2/2/api/json?tree=jobs[name,url,builds[number,url],lastSuccessfulBuild[timestamp,builtOn],lastBuild[timestamp,builtOn],actions[jobConfigHistory[currentName,date,hasConfig,job,oldName,operation,user,userID]]]"
						+ URLEncoder.encode("{0," + settings.getPageSize() + "}", "UTF-8"))),
				eq(HttpMethod.GET), Matchers.any(HttpEntity.class), eq(String.class)))
						.thenReturn(new ResponseEntity<>(getJson("instanceJobs_twoJobsTwoBuilds.json"), HttpStatus.OK));
		when(rest.exchange(eq(URI.create(
				"http://server/job/job2/2/api/json?tree=jobs[name,url,builds[number,url],jobs[name,url]]" + URLEncoder
						.encode("{" + settings.getPageSize() + "," + 2 * settings.getPageSize() + "}", "UTF-8"))),
				eq(HttpMethod.GET), Matchers.any(HttpEntity.class), eq(String.class)))
		...
		//
		// Iterator<BaseModel> buildIt =
		// jobs.get(job).get(HudsonClient.jobData.BUILD).iterator();
	...
	public void instanceJobs_multibranchPipeline() throws Exception {
		when(rest.exchange(eq(URI.create("http://server/job/job2/2/api/json?tree=jobs")), eq(HttpMethod.GET),
				Matchers.any(HttpEntity.class), eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("instanceJobs_multibranchPipeline.json"), HttpStatus.OK));
		when(rest.exchange(
				eq(URI.create(URL_TEST + "api/json?tree=jobs[name,url,builds[number,url],jobs[name,url]]"
						+ URLEncoder.encode("{0," + settings.getPageSize() + "}", "UTF-8"))),
				eq(HttpMethod.GET), Matchers.any(HttpEntity.class), eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("instanceJobs_multibranchPipeline.json"), HttpStatus.OK));

		when(rest.exchange(
				eq(URI.create("http://server/job/job1/job/master/api/json?tree=builds[number,url],jobs[name,url]")),
				eq(HttpMethod.GET), Matchers.any(HttpEntity.class), eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("instanceJobs_multibranchPipeline-master.json"), HttpStatus.OK));
		...
		when(rest.exchange(eq(URI.create(URL_TEST
				...
				+ URLEncoder.encode("{0," + settings.getPageSize() + "}", "UTF-8"))), eq(HttpMethod.GET),
				Matchers.any(HttpEntity.class), eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("instanceJobs_multibranchPipeline.json"), HttpStatus.OK));
	...
	public void buildDetails_pipeline() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class)))
						.thenReturn(new ResponseEntity<>(getJson("buildDetails_pipeline-duplicate_changeSets.json"),
								HttpStatus.OK));
		...
		assertThat(scm.getScmCommitLog(),
				is("Merged revision(s) 1651901 from lucene/dev/trunk:\nLUCENE-6177: fix typo"));
		...
		assertThat(scm.getScmCommitLog(), is(
	...
	public void buildDetails_withRepoOriginBranch() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-originBranch.json"), HttpStatus.OK));
	...
	public void buildDetails_withRepoRemotesBranch() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-remotesBranch.json"), HttpStatus.OK));
	...
	public void buildDetails_withRepoRefsRemotesBranch() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-refsRemotesBranch.json"), HttpStatus.OK));
	...
	public void buildDetails_withRepoMultipleBranches() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-multipleBranches.json"), HttpStatus.OK));
	...
	public void buildDetails_withMultipleRepos() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withMultipleRepos.json"), HttpStatus.OK));
	...
	public void buildDetails_withRepoNoRemoteUrl() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-noRemoteUrl.json"), HttpStatus.OK));
	...
	public void buildDetails_withEmptyRemoteUrl() throws Exception {
		when(rest.exchange(Matchers.any(URI.class), eq(HttpMethod.GET), Matchers.any(HttpEntity.class),
				eq(String.class))).thenReturn(
						new ResponseEntity<>(getJson("buildDetails_withRepo-emptyRemoteUrl.json"), HttpStatus.OK));
